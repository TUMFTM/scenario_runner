    ...
    *All behaviors should use this class as parent*
    ...
    def initialise(self):
        ...
        if self._actor is not None:
            self.blackboard.register_key(key="terminate_WF_actor_{}".format(self._actor.id), access=py_trees.common.Access.WRITE)
            self.blackboard.register_key(key="running_WF_actor_{}".format(self._actor.id), access=py_trees.common.Access.WRITE)
            try:
                check_attr = operator.attrgetter("running_WF_actor_{}".format(self._actor.id))
                terminate_wf = copy.copy(check_attr(self.blackboard))
...
class ChangeWeather(AtomicBehavior):
    ...
    def update(self):
        ...
        """
        self.blackboard.set("CarlaWeather", self._weather, overwrite=True)
...
class ChangeActorControl(AtomicBehavior):
    ...
    def __init__(self, actor, control_py_module, args, scenario_file_path=None, name="ChangeActorControl"):
        ...
        self.blackboard.register_key(key="ActorsWithController", access=py_trees.common.Access.WRITE)
    ...
    def update(self):
        ...
        try:
            ...
            actor_dict = check_actors(self.blackboard)
        except KeyError:
            pass
        ...
        self.blackboard.set("ActorsWithController", actor_dict, overwrite=True)
...
class AddNoiseToRouteEgo(AtomicBehavior):
    ...
    def __init__(self, actor, throttle_mean, throttle_std, steer_mean, steer_std, name="AddNoiseToVehicle"):
        ...
        self.blackboard.register_key(key="AV_control", access=py_trees.common.Access.WRITE)

        if not self.blackboard.exists('AV_control'):
            self.blackboard.set('AV_control', None, True)
    ...
    def update(self):
        ...
        control = self.blackboard.get("AV_control")
        if not control:
...
class WaypointFollower(AtomicBehavior):
    ...
    def initialise(self):
        ...
        try:
            ...
            check_attr = operator.attrgetter("running_WF_actor_{}".format(self._actor.id))
            running = check_attr(self.blackboard)
            ...
            #py_trees.blackboard.Blackboard().unset("terminate_WF_actor_{}".format(self._actor.id))
            self.blackboard.set("terminate_WF_actor_{}".format(self._actor.id), [], overwrite=False)

            #py_trees.blackboard.Blackboard().unset("running_WF_actor_{}".format(self._actor.id))
            self.blackboard.set(
                "running_WF_actor_{}".format(self._actor.id), [self._unique_id], overwrite=False)
    ...
    def update(self):
        ...
        terminate_wf = check_term(self.blackboard)
        ...
        active_wf = check_run(self.blackboard)
...
class ScenarioTriggerer(AtomicBehavior):
    ...
    def add_blackboard(self, blackboard):
        ...
        self.blackboard.register_key(blackboard[0], access=py_trees.common.Access.WRITE)
...
class SwitchWrongDirectionTest(AtomicBehavior):
    ...
    def update(self):
        self.blackboard.set("AC_SwitchWrongDirectionTest", self._active, overwrite=True)
        return py_trees.common.Status.SUCCESS
...
class SwitchMinSpeedCriteria(AtomicBehavior):
    ...
    def __init__(self, active, name="ChangeMinSpeed"):
        ...
        self.blackboard.register_key(key="SwitchMinSpeedCriteria", access=py_trees.common.Access.WRITE)
    ...
    def update(self):
        ...
        """
        new_status = py_trees.common.Status.SUCCESS
        self.blackboard.set("SwitchMinSpeedCriteria", self._active, overwrite=True)
        return new_status
...
class ScenarioTimeout(AtomicBehavior):
    ...
    def initialise(self):
        ...
        """
        self._start_time = GameTime.get_time()
        self.blackboard.set("AC_SwitchActorBlockedTest", False, overwrite=True)
        super().initialise()
    ...
    def terminate(self, new_status):
        ...
        if not self._terminated:  # py_trees calls the terminate several times for some reason.
            self.blackboard.set(f"ScenarioTimeout_{self._scenario_name}", self._scenario_timeout, overwrite=True)
            self.blackboard.set("AC_SwitchActorBlockedTest", True, overwrite=True)
            self._terminated = True
        super().terminate(new_status)
